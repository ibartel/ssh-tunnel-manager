...
var configManager = new ConfigManager(),
    ...
    profileAction = require('./actions/profile')(configManager),
    tunnelAction = require('./actions/tunnel')(configManager);
...
module.exports = require('coa').Cmd()
    ...
    .opt().name('profile').title('Specify which connection profile to use - if omitted use the default marked profile').only().val(mainAction.validators.profileExists).short('p').long('profile').end()
    .opt().name('tunnel').title('The tunnel group to run').arr().short('t').long('tunnel').end()
    //.opt().name('vars').title("Sets custom variables that can be used in tunnel configuration, format is <var>:<value>, in tunnel config you can then use {{var}}").only().arr().short('v').long('var').act(function() {
...
function tunnels() {
    this.cmd()
        ...
        .opt().name('group').title('Specify the tunnel group. Also creates if the group does not exists').short('g').long('group').req().end()
        .opt().name('sourceport').title('Specify the local (source) port').short('source').long('source-port').end()
        .opt().name('host').title('The destination host to connect to (thru the tunnel)').short('h').long('host').end()
        .opt().name('destport').title('Specify the remote (destination) port').short('dest').long('dest-port').end()
