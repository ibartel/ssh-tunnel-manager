...
module.exports = require('coa').Cmd()
    ...
    .opt().name('tunnel').title('The tunnel group(s) to run, specify multiple times or use comma separated list').arr().short('t').long('tunnel').val(tunnelAction.validators.tunnelGroup).end()
    ...
    .opt().name('file').title('Specify tunnel config file to run instead of using tunnel group of configuration').only().short('f').long('file').def('ssh-tunnels.yml').val(mainAction.validators.configFile).end()
    .completable()
...
function profiles() {
    this.cmd()
        ...
        .cmd().name('add').title('Add SSH connection profile').helpful()
            ...
            .opt().name('force').title('If the connection profile name already exists, overwrite').flag().short('f').long('force').end()
