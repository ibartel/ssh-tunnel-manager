...
module.exports = function(configManager) {
    ...
    tunnelManager.on('ssh-log', function(tunnelObj, type, msg) {
        console.log('[' + C.cyan(tunnelObj.name) + '] ' + C.blackBright(msg));
    });

    tunnelManager.on('ssh-error', function(tunnelObj, type, error) {
        if (typeof error === 'object' && error.reason && error.reason === 'CONNECT_FAILED') {
            console.log('[' + C.cyan(tunnelObj.name) + '] no response on ' + C.blackBright(tunnelObj.host + ':' + tunnelObj.destPort));
        } else {
            console.log('[' + C.cyan(tunnelObj.name) + '] ' + C.red('ERROR: ') + C.blackBright(error));
        }
    });
