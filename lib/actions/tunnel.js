    ...
    return {
        list: function() {
            ...
            if (!tunnelGroups || !tunnelGroups.length) {
                output.push(C.blackBright("There are no tunnels configured yet, use '" + utils.appName() + " tunnel add -h' to add a new one"));
            } else {
        ...
        add: function(opts, args) {
            ...
            var output = [];
            if (args.tunnelString && opts.host && opts.sourceport && opts.destport) {
                output.push(C.blackBright('Tunnel string and options specified, using the values from tunnel string'));
