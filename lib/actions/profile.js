        ...
        add: function(opts, args) {
            ...
            if (opts.default) {
                ...
                if (oldDefault) {
                    ...
                    output.push(C.blackBright('Moved default flag from connection profile ' + C.cyan(oldDefault.name)));
                } else {
                    output.push(C.blackBright('Set this profile as the default one'));
                }
            ...
            output.push('Successfully added connection profile ' + C.cyan(name) + ' to configuration file.');
            return Q.resolve(output.join('\n'));
        },
        ...
        makeDefault: function(opts, args) {
            ...
            if (!args || !args.name) {
                ...
                if (defaultProfile && defaultProfile.name && defaultProfile.name.toLowerCase() === args.name.toLowerCase()) {
                    output.push(C.blackBright('This profile is already marked as default'));
                } else {
                    if (defaultProfile && defaultProfile.name && defaultProfile.name.toLowerCase() !== args.name.toLowerCase()) {
                        output.push(C.blackBright('Moved default flag from connection profile ' + C.cyan(defaultProfile.name) + ' to ' + C.cyan(args.name)));
                    } else {
                        output.push(C.blackBright('Set default flag for connection profile ') + C.cyan(args.name));
                    }
